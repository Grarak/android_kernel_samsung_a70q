...
#if defined(CONFIG_FOLDER_HALL) && defined(CONFIG_DUAL_TSP)
	...
	for (i = 0; i < info->cap_info.multi_fingers; i++) {
		...
		if (zinitix_bit_test(sub_status, SUB_BIT_EXIST)) {
			...
			if (!m_ts_debug_mode && TSP_NORMAL_EVENT_MSG)
				input_info(true, &client->dev, "R [%02d] mc=%d\n", i, info->move_count[i]);
		}
	...
	if (!zinitix_bit_test(info->touch_info.status, BIT_PT_EXIST)) {
		for (i = 0; i < info->cap_info.multi_fingers; i++) {
			...
			if (zinitix_bit_test(prev_sub_status, SUB_BIT_EXIST)) {
				...
				input_info(true, &client->dev, "Finger[%d] up %s dd:%d,%d mc=%d (%d)\n",
...
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
				input_info(true, &client->dev, "Finger[%d] x=%d y=%d %s w=%d p=%d (0x%x)\n",
							i, x, y, location, w, palm, ic_status);
#else
				input_info(true, &client->dev, "Finger[%d] %s w=%d p=%d (0x%x)\n",
							i, location, w, palm, ic_status);
#endif
...
#else
	ssize_t count = snprintf(buf, SEC_CMD_BUF_SIZE, "ZI%02X\n",
...
static int zt75xx_power_ctrl(void *data, bool on)
{
	...
	if (!pdata->gpio_ldo_en) {
		...
		if (IS_ERR(regulator_dvdd)) {
			input_err(true, dev, "%s: Failed to get %s regulator(dvdd).\n",
...
//			return PTR_ERR(regulator_dvdd);
	...
	if (IS_ERR(regulator_avdd)) {
		input_err(true, dev, "%s: Failed to get %s regulator(avdd).\n",
...
static void zt75xx_switching_work(struct work_struct *work)
{
	...
	if (info->flip_status != info->flip_status_current) {
		...
		if (info->flip_status == 0) {
			...
			if (info->input_dev) {
				retval = mutex_lock_interruptible(&info->input_dev->mutex);
				if (retval) {
					input_err(true, &info->client->dev,
							"%s: mutex error\n", __func__);
					goto out;
			...
			if (info->input_dev) {
				...
				if (info->input_dev->users_private && info->input_dev->open) {
					retval = info->input_dev->open(info->input_dev);
					if (retval) {
						input_err(true, &info->client->dev,
							"%s: input_open failed\n", __func__);
						mutex_unlock(&info->input_dev->mutex);
				...
				info->input_dev->users = info->input_dev->users_private;
				info->input_dev->disabled = false;
