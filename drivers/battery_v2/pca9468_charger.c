...
static void pca9468_set_wdt_time(struct pca9468_charger *pca9468, int time)
{
	int ret;
	unsigned int val;
...
static void pca9468_set_done(struct pca9468_charger *pca9468, bool enable)
{
	...
	psy_swcharger = power_supply_get_by_name(pca9468->pdata->sec_dc_name);
	val.intval = enable;

	ret = power_supply_set_property(psy_swcharger,
				(enum power_supply_property) POWER_SUPPLY_EXT_PROP_DIRECT_DONE, &val);
	if (ret < 0)
...
static void pca9468_set_switching_charger(struct pca9468_charger *pca9468, bool enable)
{
	...
	val.intval = enable;
	ret = power_supply_set_property(psy_swcharger,
				(enum power_supply_property) POWER_SUPPLY_EXT_PROP_CHARGING_ENABLED_DC, &val);
	if (ret < 0)
...
#if defined(CONFIG_BATTERY_SAMSUNG)
static int pca9468_get_switching_charger_property(
...
static int pca9468_adjust_ta_current (struct pca9468_charger *pca9468)
{
	...
	if (pca9468->ta_cur == pca9468->iin_cc) {
		...
		if (pca9468->iin_cc > pca9468->pdata->iin_cfg) {
			...
			/* Set TA voltage to MAX[7500mV, (2*VBAT_ADC + 200 mV)] */
		...
		if ((val[PCA9468_REG_INT1_STS] & PCA9468_BIT_V_OK_STS) != PCA9468_BIT_V_OK_STS) {
			...
			if (val[PCA9468_REG_STS_A] & PCA9468_BIT_CFLY_SHORT_STS) {
				...
				ret = power_supply_set_property("battery",
					(enum power_supply_property) POWER_SUPPLY_EXT_PROP_VBAT_OVP, &val);
				if (ret < 0)
...
static int pca9468_preset_dcmode(struct pca9468_charger *pca9468)
{
	...
	/* Set the initial TA voltage to max[7500mV, (2*VBAT_ADC + 200 mV)] */
	pca9468->ta_vol = max(PCA9468_TA_MIN_VOL_PRESET, (2*vbat + PCA9468_TA_VOL_PRE_OFFSET));
	val = pca9468->ta_vol/PD_MSG_TA_VOL_STEP;	/* PPS voltage resolution is 20mV */
	pca9468->ta_vol = val*PD_MSG_TA_VOL_STEP;
...
#if defined(CONFIG_BATTERY_SAMSUNG)
	ret = pca9468_get_switching_charger_property(pca9468,
				(enum power_supply_property) POWER_SUPPLY_EXT_PROP_CHARGING_ENABLED_DC, &val);
#else
