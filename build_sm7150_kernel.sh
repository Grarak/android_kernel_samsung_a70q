...
DEBUG_DEFCONFIG=sm7150_sec_eng_defconfig
...
if [ "${REGION}" == "${CARRIER}" ]; then
	VARIANT_DEFCONFIG=sm7150_sec_${MODEL}_${CARRIER}_defconfig
else
	VARIANT_DEFCONFIG=sm7150_sec_${MODEL}_${REGION}_${CARRIER}_defconfig
fi
...
#			CROSS_COMPILE=$BUILD_CROSS_COMPILE dtbs || exit -1            
	$BUILD_KERNEL_DIR/tools/mkdtimg create $PRODUCT_OUT/dtbo.img --page_size=4096 $BUILD_KERNEL_OUT_DIR/arch/arm64/boot/dts/samsung/sm7150-sec-${MODEL}-overlay*.dtbo
