			       ...
			       u8 *derived_key, unsigned int derived_keysize)
{
	...
	if (S_ISREG(inode->i_mode) && 
		ctx->contents_encryption_mode == FS_ENCRYPTION_MODE_PRIVATE) {
		/* Inline encryption: no key derivation required because IVs are
		 * assigned based on iv_sector.
		 * It is confirmed by Mobile Security Technologies TG.
		 */
		memcpy(derived_key, payload->raw, sizeof(payload->raw));
		err = 0;
	} else {
